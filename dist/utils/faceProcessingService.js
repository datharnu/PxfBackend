...
class FaceProcessingService {
    ...
    static async processMediaForFaces(mediaId) {
        try {
            ...
            if (media.mediaType === "image") {
                ...
                // Detect faces in the image
                faceDetections = await googleVisionService_1.default.detectFacesFromUrl(media.mediaUrl);
            }
            ...
            let identifiedCount = 0;
            try {
                identifiedCount = await this.identifyFacesInMedia(media.eventId, faceDetectionRecords);
            }
            catch (error) {
                console.error("Error during face identification:", error);
                // Continue even if identification fails
     ...
     */
    static async processVideoForFaces(videoUrl) {
        try {
     ...
     */
    static async identifyFacesInMedia(eventId, faceDetectionRecords) {
        let identifiedCount = 0;
        // Get all user face profiles for this event
        const userFaceProfiles = await userFaceProfile_1.default.findAll({
            where: {
                eventId,
                isActive: true,
            },
        ...
        for (const faceRecord of faceDetectionRecords) {
            ...
            for (const userProfile of userFaceProfiles) {
                const similarity = this.calculateAdvancedFaceSimilarity(userProfile.faceRectangle, faceRecord.faceRectangle, userProfile.faceAttributes, faceRecord.faceAttributes);
     ...
     */
    static async findMediaWithFaceModel(eventId, faceModelData, similarityThreshold = 0.5) {
        try {
            ...
            for (const detection of faceDetections) {
                ...
                const similarity = this.calculateAdvancedFaceSimilarity(faceModelData.faceRectangle, detection.faceRectangle, faceModelData.faceAttributes, detection.faceAttributes);
                console.log(`Similarity: ${similarity.toFixed(3)} for media ${media.id}`);
                if (similarity >= similarityThreshold) {
                    ...
                    mediaMatch.overallConfidence = Math.max(mediaMatch.overallConfidence, similarity);
                }
            }
            // Sort by overall confidence (best matches first)
            const results = Array.from(matchingMedia.values()).sort((a, b) => b.overallConfidence - a.overallConfidence);
     ...
     * Get media with faces for a specific user in an event
     */
    static async getMediaWithUserFaces(eventId, userId, similarityThreshold = 0.5) {
        try {
            // Get user's face profile for this event
            ...
            return await this.findMediaWithFaceModel(eventId, {
     ...
     */
    static async uploadFaceModelAndFindMatches(eventId, faceModelImageUrl, similarityThreshold = 0.5) {
        try {
            console.log("Processing face model image:", faceModelImageUrl);
            // Detect faces in the uploaded face model image
            const faceDetections = await googleVisionService_1.default.detectFacesFromUrl(faceModelImageUrl);
            if (faceDetections.length === 0) {
                throw new Error("No faces detected in the face model image");
            }
            if (faceDetections.length > 1) {
                console.warn("Multiple faces detected, using the first one as the model");
            }
    ...
    static calculateAdvancedFaceSimilarity(face1, face2, attributes1, attributes2) {
        ...
        if (attributes1 && attributes2) {
            ...
            // Compare emotions (if available)
            if (attributes1.emotion && attributes2.emotion) {
                const emotionSimilarity = this.calculateEmotionSimilarity(attributes1.emotion, attributes2.emotion);
                attributeSimilarity += emotionSimilarity * 0.3;
            }
        }
        // Weighted combination
        const finalSimilarity = Math.min(1.0, rectangleSimilarity * 0.7 + attributeSimilarity * 0.3);
     ...
     */
    static calculateEmotionSimilarity(emotion1, emotion2) {
        const emotions = [
            "anger",
            "contempt",
            "disgust",
    ...
    static calculateFaceSimilarity(face1, face2) {
        ...
        const maxDimension = Math.max(face1.width, face1.height, face2.width, face2.height);
        const positionSimilarity = maxDimension > 0
            ? Math.max(0, 1 -
